...
# @profile
def get_pix_and_displ(halo_slice, shared_info, DDMB=None):
    ...
    if DDMB is None:
        ...
        rbin, DDMB = get_displ_vec(Mvir, cvir, tr_Mvir, tr_cvir, tr_tau, shared_info)
    ...
    for n_subhalo, sub_halo in enumerate(same_id_halos):
        ...
        np.nan_to_num(displacement, copy=False, nan=0.0, posinf=None, neginf=None)
...
def get_displ_vec(Mvir, cvir, tfNFW_Mvir, tfNFW_cvir, tfNFW_tau, shared_info):
    ...
    # get the displacement splines
    DDMB = displ(shared_info.rbin, mass['DMO_pro'], mass['DMB_pro'])
    # DDMB_tck = splrep(shared_info.rbin, DDMB, s=0, k=3)

    return shared_info.rbin, DDMB
...
def displace_shell(params, shell_particles, particle_shell_info, halos, cosmo_data, delta_shell=True, nside_out=None, n_proc=1, interp_ngrid=None):
    ...
    displ_map = np.zeros((hp.nside2npix(shell_nside), 3), dtype=np.float32)
    ...
    for i in LOGGER.progressbar(range(len(halo_splits) - 1), at_level='info', desc=f'processing halos in shell {particle_shell_info["shell_id"]}'):
        ...
        if interp_ngrid is None:
            ...
            sub_info, pixels, displacement = get_pix_and_displ(halos_slice, current_info, DDMB=None)

        else:
            
            sub_info, pixels, displacement = get_pix_and_displ(halos_slice, current_info, DDMB=ddmb_interp[i])
            # sub_info, pixels, displacement = get_pix_and_displ_interp(ddmb_interp[i], halos_slice, current_info)

            # accuracy check 
            if i % 10000 == 0:
                sub_info_nointerp, pixels_nointerp, displacement_nointerp = get_pix_and_displ(halos_slice, current_info, DDMB=None)
                check_displacement_interpolation_accuracy(pixels_nointerp, displacement_nointerp, pixels, displacement)
    ...
    displ_norm = np.linalg.norm(displ_map, axis=1)
    shell_info["n_parts_unique"] = np.sum(displ_norm > 1e-8)

    # get the relevant displacements
    displ_mask = np.logical_and(shell_particles > 0.5, displ_norm)
    relevant_displ = displ_map[displ_mask]
    relevant_pix = np.arange(hp.nside2npix(shell_nside))[displ_mask]

    # create positions and relative distance to halo
    parts_pos = np.stack(hp.pix2vec(nside=shell_nside, ipix=relevant_pix), axis=1)
    # blow up to radius
    parts_pos *= current_com

    # displace
    parts_pos += relevant_displ

    # normalize
    parts_r = np.linalg.norm(parts_pos, ord=2, axis=1, keepdims=True)
    parts_pos /= parts_r
    np.nan_to_num(parts_pos, copy=False, nan=0.0, posinf=None, neginf=None)

    # project with interpolation
    theta, phi = hp.vec2ang(parts_pos)
    # pixels and weights have shape (4, N)
    pixels, weights = hp.get_interp_weights(nside=shell_nside, theta=theta, phi=phi)
    # transpose to (N, 4)
    pixels = pixels.T
    weights = weights.T.astype(np.float32)

    # asign
    displ_shell = np.zeros(hp.nside2npix(shell_nside), dtype=np.float32)
    numba_asign(displ_shell, pixels, weights, shell_particles[relevant_pix])

    if delta_shell:
        # get the difference
        displ_shell[relevant_pix] -= shell_particles[relevant_pix]
    else:
        # add the irrelevant pixel
        inv_mask = np.logical_not(displ_mask)
        displ_shell[inv_mask] += shell_particles[inv_mask]

    if nside_out is not None:
        if nside_out != shell_nside:
            displ_shell = hp.ud_grade(displ_shell, nside_out=nside_out, power=-2)

    return particle_shell_info, shell_info, displ_shell
...
def get_baryon_shells(param, shells, shells_info, halos, delta_shell=True, nside_out=None, test=False, interp_halo_displacements=False, interp_ngrid=int(2**13)):
    ...
    for i, h in LOGGER.progressbar(list(enumerate(shelled_halos)), desc='displacing shells', at_level='warning'):
        ...
        shell_info, shell_stats, baryon_shell = displace_shell(params=param, 
    ...
    return baryon_shells, shells_stats_arr, shells_back
